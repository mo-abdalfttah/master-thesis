...
DimPlot(Data, group.by = "SCT_snn_res.0.5", cols =cols)
SpatialDimPlot(Data, pt.size.factor = 2,group.by = "SCT_snn_res.0.5") + 
...
SpatialFeaturePlot(Data, features = c("PTPRC", "CD3E", "CD19", "CD14", # Immune Cells
                                      "EPCAM", "CDH1", "KRT18", "MUC2","REG1A", "REG1B", "CEACAM6", "TGFBI", # Epithilial Cells
                                      "PECAM1", "VWF", "CD34", "SELE", #Enothilial cells 
                                      "VIM", "FAP", "DCN", "ACTA2" # Stromal Cells
                                      ), max.cutoff = "q95", ncol = 4, alpha = c(0, 1), crop = F) & scale_fill_viridis_c() 
FeaturePlot(Data, features = c("PTPRC", "CD3E", "CD19", "CD14",
                                      "EPCAM", "CDH1", "KRT18", "MUC2","REG1A", "REG1B", "CEACAM6", "TGFBI",
                                      "PECAM1", "VWF", "CD34", "SELE",
                                      "VIM", "FAP", "DCN", "ACTA2"), max.cutoff = "q95") +
  DimPlot(Data, cols = ggsci::pal_igv()(50), group.by = "SCT_snn_res.0.5") +
  SpatialDimPlot(Data, label = FALSE, group.by = "SCT_snn_res.0.5") + 
    guides(fill = guide_legend(override.aes = list(size=10))) & 
    scale_fill_manual(values = ggsci::pal_igv()(50))
...
VlnPlot(Data, features = c("PTPRC", "CD3E", "CD19", "CD14",
                                      ...
                                      "VIM", "FAP", "DCN", "ACTA2")) +
  DimPlot(Data, cols = ggsci::pal_igv()(50), group.by = "SCT_snn_res.0.5") +
  SpatialDimPlot(Data, label = FALSE, pt.size.factor = 2.5, group.by = "SCT_snn_res.0.5") + 
    guides(fill = guide_legend(override.aes = list(size=10))) & 
    scale_fill_manual(values = ggsci::pal_igv()(50))
...
Data@meta.data <- Data@meta.data %>%
    dplyr::mutate(
        annotation_lvl1 = dplyr::case_when(
            SCT_snn_res.0.5 == 0 ~ "Epithelial-Normal",
            SCT_snn_res.0.5 == 1 ~ "TME",
            SCT_snn_res.0.5 == 2 ~ "TME",
            SCT_snn_res.0.5 == 3 ~ "Epithelial",
            SCT_snn_res.0.5 == 4 ~ "Epithelial-Normal",
            SCT_snn_res.0.5 == 5 ~ "TME",
            SCT_snn_res.0.5 == 6 ~ "Epithelial"
